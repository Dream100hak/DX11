...
void AsConverter::ExportModelData(wstring savePath)
{
...
void AsConverter::ReadMeshData(aiNode* node, int32 bone)
{
...
void AsConverter::WriteModelFile(wstring finalPath)
{
...
std::string AsConverter::WriteTexture(string saveFolder, string file)
{
	...
	if (srcTexture)
	{
		...
		else
		{
			...
			::CaptureTexture(DEVICE.Get(), DCT.Get(), texture.Get(), img);
...
std::shared_ptr<asAnimation> AsConverter::ReadAnimationData(aiAnimation* srcAnimation)
{
...
std::shared_ptr<asAnimationNode> AsConverter::ParseAnimationNode(shared_ptr<asAnimation> animation, aiNodeAnim* srcNode)
{
...
void AsConverter::ReadKeyframeData(shared_ptr<asAnimation> animation, aiNode* srcNode, map<string, shared_ptr<asAnimationNode>>& cache)
{
...
void AsConverter::WriteAnimationData(shared_ptr<asAnimation> animation, wstring finalPath)
{
	auto path = filesystem::path(finalPath);
...
uint32 AsConverter::GetBoneIndex(const string& name)
{
	for (shared_ptr<asBone>& bone : _bones)
	{
		if (bone->name == name)
