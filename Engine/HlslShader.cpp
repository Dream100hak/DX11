...
// --------------------------------------------------------------------------
// Create : HlslShaderDesc�?받아 �??�이?��? ?�성?�는 ?�수
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Push ?�수??(공통 Shader ?�라미터 ?�정, b0~b6, VS+PS ?�시 바인??
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Internal : VS Blob 리플?�션?�로 InputLayout ?�동 ?�성
// --------------------------------------------------------------------------
void HlslShader::CreateInputLayoutFromVS(ComPtr<ID3DBlob> vsBlob)
{
	...
	for (UINT i = 0; i < shaderDesc.InputParameters; i++)
	{
		...
		// INST* (Instancing) ?�는 PICKED??InputSlot 1???�당
		...
		// INST_WORLD (matrix): 4개의 float4�?분리 - 리플?�션??index 0~3???�해
		// index 0?�서�?4�?추�??�고, ?�머지 index(1~3)???�킵
		if (name == "INST_WORLD")
		{
			...
			continue;  // index 0~3 모두 ?�킵 (?�에???��? 4�?추�???
		}

		inputLayout.push_back(elem);
	}

	// InputLayout???�는 경우?�만 ?�성
