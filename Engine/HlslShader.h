...
struct HlslShaderDesc
{
	...
	// ---- Stream-Output (?�티???? ----
	// soEntries 가 비어?��? ?�으�?GS �?CreateGeometryShaderWithStreamOutput ?�로 ?�성.
	// FX ??ConstructGSWithSO("POS.xyz; ...") ?��?
	vector<D3D11_SO_DECLARATION_ENTRY> soEntries;
	uint32 soStride = 0;          // SO 버퍼(?�롯 0) ?�점 ?�트?�이??
	bool   soRasterize = false;   // false = SO ?�용 (?�스?�라?�즈 ????
};
...
public:
	...
	void DrawAuto(); // Stream-Output 결과 ?�로??(?�점 ???�동)
	...
	void PushModelBoneData(const Matrix& boneTransform); // b5: ModelRenderer per-mesh �?
