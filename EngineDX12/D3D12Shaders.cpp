...
// 인라인 HLSL 셰이더 소스 정의 (D3D12Device.cpp 에서 분리). CreatePipeline 이 참조.
const char* kSceneCB = R"(
...
const std::string kMeshShader = std::string(kSceneCB) + R"(
#define OCT 8
...
const std::string kGatherShader = std::string(kSceneCB) + R"(
...
const std::string kSkyShader = std::string(kSceneCB) + R"(
...
const std::string kGridShader = std::string(kSceneCB) + R"(
struct VOut { float4 pos:SV_POSITION; float2 clip:TEXCOORD0; };
VOut VSMain(uint id : SV_VertexID)
{
...
const std::string kOutlineShader = std::string(kSceneCB) + R"(
...
const std::string kParticleShader = std::string(kSceneCB) + R"(
...
const std::string kTessShader = std::string(kSceneCB) + R"(
...
const std::string kWaterShader = std::string(kSceneCB) + R"(
...
const std::string kProbeViz = std::string(kSceneCB) + R"(
